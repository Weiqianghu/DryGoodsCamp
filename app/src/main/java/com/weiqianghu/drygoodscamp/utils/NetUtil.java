...
public class NetUtil {
    public static int getNetWorkType() {
        int netWorkType = Constant.NETWORKTYPE_INVALID;
        ...
        if (networkInfo != null && networkInfo.isConnected()) {
            ...
            if (type.equalsIgnoreCase("WIFI")) {
                netWorkType = Constant.NETWORKTYPE_WIFI;
            } else if (type.equalsIgnoreCase("MOBILE")) {
                String proxyHost = android.net.Proxy.getDefaultHost();

                netWorkType = TextUtils.isEmpty(proxyHost) ? (isFastMobileNetwork(App.getContext()) ? Constant.NETWORKTYPE_3G : Constant.NETWORKTYPE_2G) : Constant.NETWORKTYPE_WAP;
            }
        } else {
            netWorkType = Constant.NETWORKTYPE_INVALID;
        }
